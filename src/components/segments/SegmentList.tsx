...
export function SegmentList() {
  ...
  const ensureElementVisibleWithinContainer = useCallback((target: HTMLElement) => {
    ...
    const container = getNearestScrollableAncestor(target) ?? (document.scrollingElement as HTMLElement | null) ?? document.body;
    ...
    const margin = 16;
    const targetCenterOffset = targetRect.top - containerRect.top - (container.clientHeight / 2) + (targetRect.height / 2);
    const desiredTop = Math.max(0, container.scrollTop + targetCenterOffset - margin);
